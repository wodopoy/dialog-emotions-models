...
CEDR test, `cedr_test` = вторая половина** (непересекающиеся, обе held-out от весов). Фит — free
(без дедбанда), чтобы эффект dev-набора был чистым; `raw` = T = 1.
...
| rubert-tiny2-finetune | 0.054 | **0.013** | 0.038 | 0.023 |
| logreg-char | **0.028** | 0.032 | 0.078 | 0.032 |
| fasttext | **0.016** | 0.017 | 0.104 | 0.022 |
...
| fasttext | 0.123 | 0.116 | **0.026** | 0.109 |
| hf-fyaronskiy-deberta | 0.104 | 0.063 | **0.021** | 0.053 |
| logreg-char | 0.067 | 0.061 | **0.033** | 0.061 |
...
| rubert-tiny2-finetune | 0.042 | 0.020 | 0.027 | **0.014** |
| logreg-word | 0.040 | 0.024 | 0.036 | **0.021** |
| logreg-char | **0.023** | 0.027 | 0.064 | 0.027 |

Закономерность чистая: **лучший dev = тот, что совпадает с доменом оценки.**

- **`T_rugoval`** — лучший на RuGo test (по построению); `T_cedrval` там стабильно худший.
...
- **`T_comb`** — лучший на совокупном тесте и почти-лучший на каждом домене по отдельности:
  **лучший единый компромисс**. Но он наследует RuGo-перекос (и dev, и совокупный test на ~85%
  ...
  `T_cedrval`.
- **Ни один скаляр T не оптимален на всех трёх сразу:** `T_cedr` жертвует RuGo (logreg-char
  0.032 → 0.078), `T_rugo` — CEDR. Поэтому «просто калибровать на объединении» — не бесплатный
  обед: совокупность RuGo-доминирована, и `T_comb` тянется к RuGo, недокалибровывая нативный домен.
