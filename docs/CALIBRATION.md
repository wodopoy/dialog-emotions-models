...
## Калибровать на RuGo, CEDR или совокупности?

Законный вопрос: модели обучаются на **RuGo + CEDR**, а T подбирается на **RuGo val** — почему
не на совокупности, на которой учимся? Две причины: (1) у CEDR нет валидационного сплита (только
train, ушедший в веса, и test), поэтому единственный held-out был RuGo val; (2) — и это главное
— **наивная «совокупность» почти равна RuGo**, потому что в смеси translationese-RuGo численно
давит нативный CEDR ~6:1, и редкий целевой сигнал в ней тонет.

Проверено честно: CEDR test разрезан 50/50 на **calib** (фит T) и **report** (оценка), фит и
репорт не пересекаются. Для каждой модели — три температуры: `T_rugo` (RuGo val), `T_cedr`
(CEDR calib), `T_combined` (RuGo val + CEDR calib, натуральные размеры). ECE на CEDR report
(деплой-домен):

| модель | CEDR ECE raw | @T_rugo | @T_combined | **@T_cedr** | RuGo ECE @T_rugo→@T_cedr |
|---|---|---|---|---|---|
| hf-maxkazak-rubert-base | 0.226 | 0.183 | 0.164 | **0.074** | 0.014 → 0.195 |
| ridge-char | 0.164 | 0.062 | 0.062 | **0.053** | 0.025 → 0.040 |
| fasttext | 0.123 | 0.116 | 0.109 | **0.026** | 0.017 → 0.104 |
| hf-fyaronskiy-deberta | 0.104 | 0.063 | 0.053 | **0.021** | 0.027 → 0.039 |
| logreg-char | 0.067 | 0.061 | 0.061 | **0.033** | 0.032 → 0.078 |

Что видно:

- **`T_combined` ≈ `T_rugo`** почти везде (CEDR ECE под ними совпадает: logreg-char 0.061=0.061,
  ridge-char 0.062=0.062). Натуральная совокупность RuGo-доминирована (val 5374 против CEDR
  calib 941, ~5.7:1), поэтому «калибровать на объединении» ≡ «калибровать на RuGo» — целевой
  домен это не чинит. **Это и есть ответ: дело не в том, что я выбрал RuGo вместо совокупности —
  на натуральной совокупности вышло бы то же самое.**
- **Целевой `T_cedr` реально чинит деплой-домен:** logreg-char 0.067 → 0.033, fasttext
  0.123 → **0.026**, deberta 0.104 → **0.021**, maxkazak 0.226 → 0.074.
- **Цена — хуже на RuGo** (logreg-char RuGo 0.032 → 0.078; fasttext 0.017 → 0.104). Один скаляр
  T не может быть оптимален для обоих доменов сразу — у них разные профили уверенности
  (logreg-char хочет T≈0.83 на CEDR против ≈0.98 на RuGo; fasttext 0.68 против 0.98).
- **Балансировка** совокупности в сторону цели (50/50) помогает там, где есть реальная
  CEDR-переуверенность (fasttext CEDR 0.109 → 0.059), но шумна для уже калиброванных моделей
  (для logreg-char балансировка по 941 примеру ловит шум). Надёжнее фитить прямо на целевом dev.

**Вывод для деплоя:** раз продукт работает на нативном русском, калибровать T надо на
**целевом (CEDR-подобном) dev**, а не на RuGo и не на RuGo-доминированной совокупности. RuGo-val
в основном прогоне — компромисс из-за отсутствия CEDR-val; этот раздел показывает, что́ даёт
правильный выбор. Идеал (повторяющийся вывод работы): собрать нативные **диалоговые** данные
целевого домена и калибровать (и дообучать) на них; при необходимости — **per-domain T**.

Воспроизвести: `python scripts/calibrate_combined.py` →
